...
class Perceptron:
    ...
    def generate_random_weight(self, num_elem):
        return map(lambda r: r - 0.5, random.rand(num_elem))

    def insert_dummy_elem(self, x):
    ...
    def execute(self, training_data, training_labels):
        training_data_with_dummy = self.insert_dummy_elem(training_data)
        w = self.generate_random_weight(len(training_data_with_dummy[0]))

        for time in range(100):
            cnt_err = 0
            for xi, labeli in zip(training_data_with_dummy, training_labels):
                update = self.eta * (labeli - self.calculate_label(xi, w))
    ...
    def calculate_labels(self, data, w):
        data_with_dummy = self.insert_dummy_elem(data)
        labels = []
        for j in range(len(data_with_dummy)):
            label = self.calculate_label(data_with_dummy[j], w)
            labels.append(label)
        return labels

    def calculate_label(self, data_i, w):
        z = 0
...
class PerceptronTest:
    ...
    def test(self):
        ...
        w = self.ppn.execute(training_data, training_labels)
        labels = self.ppn.calculate_labels(test_data, w)
        self.ppn.print_result(test_data, labels)
    ...
    def do_test(self, data, labels):
        ...
        w = self.ppn.execute(training_data, training_labels)
        result_labels = self.ppn.calculate_labels(test_data, w)
